...
int validate(char *buf, int buf_top, int text_head, int wrong_count) {
  if (buf[buf_top - 1] == 8 || buf[buf_top - 1] == 127)
    return 3;
  if (wrong_count)
    return 0;
  if ((game_text[text_head + 1] == '\0' || game_text[text_head + 1] == EOF) &&
      game_text[text_head] == buf[buf_top - 1])
    return 2;
  return game_text[text_head] == buf[buf_top - 1];
}

/* Takes a text buffer and insert carriage-return+newline-combinations
 * if the text reaches the end of the line.
...
void print_text_with_caret(char *text, int position, int wrong_count) {
  ...
  while (!end) {
    ...
    for (; text[i] != '\n' && text[i] != '\0' && text[i] != EOF; i++, col++) {
      if (i >= position && i < position + wrong_count && wrong_count > 0)
        printf("%s%c", KRED, text[i] == ' ' ? '-' : text[i]); // Make text red
      else if (i < position)
        printf("%s%c", KGRN, text[i]); // Make text green
      else
        printf("%s%c", KNRM, text[i]); // Make text normal color
    }
